<script lang="ts">
    ...
    const notYourAddr = '0x00000000000000000000000000';

    async function sign() {
        nonce = Math.floor(Math.random() * 1000000000);
        signature = await web3Props.signer.signMessage(`I am signing my one-time nonce: ${nonce}`);
        signed = true;
    }

    function verify() {
        address = ethers.verifyMessage(`I am signing my one-time nonce: ${nonce}`, signature);
